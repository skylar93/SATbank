...
import { calculateDashboardStats, type ResultsDashboardData } from '../../../lib/results-service'
...
export default function ResultsDashboardClient({ initialData }: ResultsDashboardClientProps) {
  const { user } = useAuth()
    ...
    <div className="h-full bg-gray-50">
      ...
      <div className="p-6">
              ...
              {user?.profile?.target_score && stats.bestScore && (
                <div>
                  <div className="flex justify-between text-sm text-gray-600 mb-1">
                    <span>Progress</span>
                    <span>{Math.round((stats.bestScore / user.profile.target_score) * 100)}%</span>
                  </div>
                  <Progress 
                    value={Math.min((stats.bestScore / user.profile.target_score) * 100, 100)} 
                    className="h-2"
                  />
                </div>
              )}
        ...
        <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
                <div className="p-6 border-b border-gray-100">
                  ...
                  <p className="text-gray-600 text-sm mt-1">
                    Click on any row to expand and view detailed scores and actions
                  </p>
                </div>
                <Table>
                  <TableHeader>
            ...
            {stats.completedAttempts.length > 0 && stats.visibleCompletedAttempts.length === 0 && (
