...
} from '../../../lib/weekly-activity-service'
...
export default function StudentDashboard() {
  ...
  const [weeklyActivityData, setWeeklyActivityData] =
    ...
    try {
      if (user) {
        ...
        setStats({
          ...
          recentAttempts,
          canShowResults,
          previousMonthBestScore: canShowResults
            ? previousMonthStats.bestScore
            : null,
          previousMonthExamsTaken: previousMonthStats.examsTaken,
          previousMonthAverageScore: canShowResults
            ? previousMonthStats.averageScore
            : null,
        })
        ...
        setSubjectData(
          canShowResults ? subjectScores : { reading: 0, writing: 0, math: 0 }
        )
      }
    } catch (error) {
    } finally {
  ...
  const fetchPreviousMonthStats = async (userId: string) => {
    ...
    const scores = attempts
      .map((attempt) => {
        return (
          (attempt as any).final_scores?.overall || attempt.total_score || 0
        )
      })
  ...
  const fetchUserSubjectScores = async (
    userId: string
    ...
    answers.forEach((answer: any) => {
      ...
      if (question && question.module_type) {
        ...
        if (answer.is_correct) {
          moduleStats[question.module_type as keyof typeof moduleStats]
            .correct++
        }
      }
    })
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              <StatsCard
                ...
                changeType={(() => {
                  ...
                  return stats.bestScore &&
                    stats.previousMonthBestScore &&
                    stats.bestScore >= stats.previousMonthBestScore
                    ? 'positive'
                    : 'negative'
                })()}
                miniChart={{
                  data:
                    stats.canShowResults && scoreHistory.length > 0
                      ? scoreHistory.slice(-6).map((item) => item.score)
                      : [0, 0, 0, 0, 0, 0],
                  color: '#10b981',
                }}
              />
              ...
              <StatsCard
                ...
                changeType={(() => {
                  ...
                  if (result.isZero) return 'neutral'
                  return stats.examsTaken >= stats.previousMonthExamsTaken
                    ? 'positive'
                    : 'negative'
                })()}
                miniChart={{
                  data: Array.from({ length: 6 }, (_, i) =>
                    Math.max(0, stats.examsTaken - 5 + i)
                  ),
                  color: '#8b5cf6',
                }}
              />
              ...
              <StatsCard
                ...
                value={
                  loading
                    ...
                    : !stats.canShowResults
                      ? 'Results Hidden'
                      : stats.averageScore || 'No scores yet'
                }
                change={(() => {
                  const result = calculatePercentageChange(
                    stats.averageScore,
                    stats.previousMonthAverageScore
                  )
                  return result.change
                })()}
                changeType={(() => {
                  ...
                  return stats.averageScore &&
                    stats.previousMonthAverageScore &&
                    stats.averageScore >= stats.previousMonthAverageScore
                    ? 'positive'
                    : 'negative'
                })()}
                miniChart={{
          ...
          <div className="lg:col-span-3 space-y-4 md:space-y-6">
            {/* Calendar */}
            <Calendar
              events={studyStreakDays.map((date) => ({
                date: new Date(date),
                type: 'visit' as const,
              }))}
            />
            ...
            <div className="bg-white rounded-2xl shadow-sm p-4 md:p-6">
              ...
              <div className="flex justify-center mb-4 md:mb-6">
                <CircularProgress
                  percentage={
                    stats.canShowResults && stats.bestScore
                      ? Math.round((stats.bestScore / 1600) * 100)
                      : 0
                  }
                  size={120}
                />
