...
async function getDashboardData(userId: string): Promise<{ data: DashboardData; canShowResults: boolean }> {
  const supabase = createServerComponentClient({ cookies })
  
  try {
    // Try using the new RPC function first
    const { data: rpcData, error: rpcError } = await supabase.rpc('get_student_dashboard_data', {
      p_user_id: userId
    })
    ...
    if (!rpcError && rpcData) {
      ...
      if (rpcData.recentAttempts.length > 0) {
        ...
        if (mostRecentExam.exam_id) {
          try {
            canShowResults = await ExamService.canShowResults(userId, mostRecentExam.exam_id)
          } catch (error) {
            canShowResults = true
          }
        }
...
async function fetchUserSubjectScores(userId: string): Promise<{ reading: number; writing: number; math: number }> {
  const supabase = createServerComponentClient({ cookies })
...
export default async function StudentDashboard() {
  ...
  const { data: dashboardData, canShowResults } = await getDashboardData(user.id)
