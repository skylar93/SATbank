...
export default function DebugRLSPage() {
  ...
  const runRLSTests = async () => {
    ...
    try {
      ...
      const { data: practiceDebug, error: practiceDebugError } = await supabase
        .rpc('debug_practice_creation', { test_user_id: user?.id })
      ...
      const { data: practiceInsert, error: practiceInsertError } = await supabase
        .rpc('test_practice_insert', { test_user_id: user?.id })

      testResults.push({
        test: 'Practice Insert Test',
        success: !practiceInsertError && practiceInsert?.[0]?.success,
        error: practiceInsertError?.message || practiceInsert?.[0]?.error_message,
        data: practiceInsert
      })
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Debug RLS Page</h1>
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold text-gray-900">RLS Debug Tests</h1>
              ...
              >
                ...
                {result.authUid && (
                  <div className="mb-2">
                    ...
                    <div className={`text-sm font-medium ${result.match ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="mt-8 bg-gray-50 rounded-lg p-4">
            <h3 className="font-medium text-gray-900 mb-2">Current Context Info</h3>
          ...
          <div className="mt-4 text-sm text-gray-600">
            <p><strong>Purpose:</strong> This page tests RLS policies and auth state to identify why practice session creation is failing.</p>
            <p><strong>Check:</strong> Look for mismatches between auth.uid() and context user.id</p>
          </div>
        </div>
      </div>
    </div>
