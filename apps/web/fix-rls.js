...
async function fixRLS() {
  ...
  try {
    ...
    const { error: dropError } = await supabase.rpc('sql', {
      query: 'DROP POLICY IF EXISTS "Users can view questions for active exams" ON questions;'
    })
    
    if (dropError) {
      console.log('Note: Could not drop old policy (may not exist):', dropError.message)
    }
    ...
    if (alterError) {
      console.log('Note: Could not alter exam_id column (may already be nullable):', alterError.message)
    }
