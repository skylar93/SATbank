...
export function parseTableFromMarkdown(text: string): { headers: string[], rows: string[][] } | null {
  if (!text) return null;
...
export function buildTableMarkdown(tableData: { headers: string[], rows: string[][] }): string {
  let markdown = '{{table}}\n';
