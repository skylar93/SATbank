...
export class AnalyticsService {
  ...
  private static readonly SCORE_CONVERSION = {
    ...
    reading: [
      ...
      650, 660, 670, 680, 690, 700, 720, 740, 760, 780, 800,
    ],
    math: [
      200, 210, 220, 230, 240, 250, 260, 270, 280, 290, 300, 310, 320, 330, 340,
      350, 360, 370, 380, 390, 400, 410, 420, 430, 440, 450, 460, 470, 480, 490,
  ...
  static async getComprehensiveResults(
    attemptId: string
    ...
    const detailedScore = attempt.final_scores
      ? this.buildDetailedScoreFromFinalScores(
  ...
  static buildDetailedScoreFromFinalScores(
    finalScores: any,
    answers: any[],
    questions: Question[]
  ): DetailedScore {
    ...
    const percentages = {
      english1:
        moduleStats.english1.total > 0
          ? (moduleStats.english1.correct / moduleStats.english1.total) * 100
          : 0,
      english2:
        moduleStats.english2.total > 0
          ? (moduleStats.english2.correct / moduleStats.english2.total) * 100
          : 0,
      math1:
        moduleStats.math1.total > 0
          ? (moduleStats.math1.correct / moduleStats.math1.total) * 100
          : 0,
      math2:
        moduleStats.math2.total > 0
          ? (moduleStats.math2.correct / moduleStats.math2.total) * 100
          : 0,
      overall: 0,
    }

    const totalCorrect = Object.values(rawScores).reduce(
      (sum, score) => sum + score,
      0
    )
    const totalQuestions = Object.values(moduleStats).reduce(
  ...
  static calculateDetailedScore(
    ...
    const readingScore = this.convertToSATScore(
      rawScores.english1,
    ...
    const writingScore = this.convertToSATScore(
      rawScores.english2,
  ...
  private static convertToSATScore(
    correct: number,
    total: number,
    ...
    const index = Math.min(
      Math.floor(percentage * (conversionTable.length - 1)),
      conversionTable.length - 1
    )
  ...
  private static buildQuestionAnalysis(
    answers: any[],
    questions: Question[]
  ): QuestionAnalysis[] {
    return answers
  ...
  private static calculatePerformanceAnalytics(
    answers: any[],
    questions: Question[]
  ): PerformanceAnalytics {
    ...
    const strengthAreas = topicPerformance
      ...
      .map((t) => t.topic)
    const weaknessAreas = topicPerformance
  ...
  private static async getTestAttempt(
    attemptId: string
  ...
  private static async getAllQuestionsForAttempt(
    attemptId: string
  ): Promise<Question[]> {
    const { data, error } = await supabase
      .from('user_answers')
  ...
  private static async calculateProgressComparison(
    userId: string,
    currentAttemptId: string
  ) {
    const attempts = await ExamService.getUserAttempts(userId)
    const completedAttempts = attempts.filter(
      (a) => a.status === 'completed' && a.id !== currentAttemptId
    )
    ...
    const scoreImprovement =
      currentAttempt.total_score - (latestPrevious.total_score || 0)
  ...
  static async getDashboardOverallStats(userId: string): Promise<{
    ...
    const uniqueExamIds = [
      ...new Set(
        completedAttempts.filter((a) => a.exam_id).map((a) => a.exam_id!)
      ),
    ]
  ...
  static async getDashboardScoreHistory(userId: string): Promise<
    Array<{
      date: string
      score: number
    }>
