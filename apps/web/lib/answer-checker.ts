...
export function normalizeCorrectAnswers(correctAnswer: any): string[] {
  ...
  if (typeof correctAnswer === 'object' && correctAnswer !== null) {
    try {
      const parsed = Array.isArray(correctAnswer) ? correctAnswer : JSON.parse(JSON.stringify(correctAnswer))
      return Array.isArray(parsed) ? parsed.filter(answer => typeof answer === 'string') : []
    } catch {
      return []
    }
  }
