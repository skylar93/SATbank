...
class AuthStateManager {
  ...
  private notifySubscribers(user: AuthUser | null) {
    this.subscribers.forEach(callback => {
      try {
        ...
        console.error('❌ AuthStateManager: Error in subscriber callback:', error)
      }
    })
  }
  ...
  private async getProfileWithRetry(userId: string, retries = 2): Promise<{ data: UserProfile | null; error: any }> {
    for (let i = 0; i <= retries; i++) {
      try {
        ...
        if (i < retries) {
          console.log(`🔄 AuthStateManager: Profile fetch failed, retrying... (${i + 1}/${retries})`)
        ...
        if (i < retries) {
          console.log(`🔄 AuthStateManager: Profile fetch error, retrying... (${i + 1}/${retries})`)
          await new Promise(resolve => setTimeout(resolve, 1000))
        } else {
  ...
  async getCurrentUser(forceRefresh = false): Promise<AuthUser | null> {
    ...
    if (!forceRefresh && this.isCacheValid()) {
      console.log('🎯 AuthStateManager: Returning cached user:', this.cache!.user?.email || 'none')
      return this.cache!.user
    }

    // If there's already a request in progress, wait for it
    if (this.currentRequest) {
      console.log('⏳ AuthStateManager: Request already in progress, waiting...')
    ...
    try {
      ...
      console.log('✅ AuthStateManager: User fetched and cached:', user?.email || 'none')
      return user
    } finally {
  ...
  private async fetchCurrentUser(): Promise<AuthUser | null> {
    ...
    try {
      ...
      if (sessionError) {
        console.warn('⚠️ AuthStateManager: Session error:', sessionError.message)
      }
      ...
      if (this.cache && (error.message.includes('timeout') || error.message.includes('fetch'))) {
        console.log('🔄 AuthStateManager: Using cached data due to network error')
        return this.cache.user
      }
  ...
  private async buildAuthUser(user: any): Promise<AuthUser> {
    ...
    if (profileError && profileError.code !== 'PGRST116') {
      console.warn('⚠️ AuthStateManager: Profile fetch failed:', profileError.message)
    }
  ...
  handleAuthStateChange(event: string, session: any) {
    console.log('🔄 AuthStateManager: Auth state change:', event, session?.user?.email || 'no user')
