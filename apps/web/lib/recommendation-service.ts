...
export class RecommendationService {
  ...
  static async getStudentWrongAnswers(userId: string, limit?: number): Promise<WrongAnswer[]> {
  ...
  static async analyzeWeaknesses(userId: string): Promise<WeaknessAnalysis> {
    ...
    // Topic analysis
    const topicStats: Record<string, { attempted: number; correct: number; difficulties: number[]; recent: number }> = {}
    ...
    allAnswers.forEach(answer => {
      ...
      if (question && question.topic_tags) {
        question.topic_tags.forEach((topic: string) => {
          if (!topicStats[topic]) {
            topicStats[topic] = { attempted: 0, correct: 0, difficulties: [], recent: 0 }
          }
          topicStats[topic].attempted++
          topicStats[topic].difficulties.push(this.difficultyToNumber(question.difficulty_level))
    ...
    const weakTopics = Object.entries(topicStats)
      .map(([topic, stats]) => ({
        ...
        accuracyRate: stats.attempted > 0 ? (stats.correct / stats.attempted) * 100 : 0,
        avgDifficulty: stats.difficulties.length > 0 ? stats.difficulties.reduce((sum, d) => sum + d, 0) / stats.difficulties.length : 0,
        recentMistakes: stats.recent
      }))
      .filter(topic => topic.accuracyRate < 70 && topic.questionsAttempted >= 3)
    ...
    // Module analysis
    const moduleStats: Record<ModuleType, { attempted: number; correct: number; timeSpent: number; mistakes: string[] }> = {
    ...
    const moduleWeaknesses = Object.entries(moduleStats)
      .map(([module, stats]) => ({
        ...
        accuracyRate: stats.attempted > 0 ? (stats.correct / stats.attempted) * 100 : 0,
        avgTimePerQuestion: stats.attempted > 0 ? stats.timeSpent / stats.attempted : 0,
        commonMistakes: [...new Set(stats.mistakes)].slice(0, 5)
      }))
      .filter(module => module.accuracyRate < 75)

    // Time management analysis
    const totalTime = allAnswers.reduce((sum, a) => sum + (a.time_spent_seconds || 0), 0)
    const avgTimePerQuestion = allAnswers.length > 0 ? totalTime / allAnswers.length : 0
  ...
  static async generateRecommendations(userId: string): Promise<StudyRecommendation[]> {
    ...
    weaknessAnalysis.weakTopics.slice(0, 3).forEach(topic => {
      recommendations.push({
        type: 'topic',
        priority: topic.accuracyRate < 50 ? 'high' : topic.accuracyRate < 65 ? 'medium' : 'low',
        ...
        estimatedStudyTime: Math.max(30, Math.round((100 - topic.accuracyRate) * 2)),
        relatedQuestions: [] // Would be populated with relevant question IDs
      })
    })

    // Difficulty-based recommendations
    Object.entries(weaknessAnalysis.difficultyStruggles).forEach(([difficulty, stats]) => {
      if (stats.rate < 70 && stats.attempted >= 5) {
        recommendations.push({
          type: 'difficulty',
          priority: difficulty === 'easy' ? 'high' : difficulty === 'medium' ? 'medium' : 'low',
          title: `Improve ${difficulty} question accuracy`,
          description: `Your accuracy on ${difficulty} questions is ${stats.rate.toFixed(1)}%. ${
            difficulty === 'easy' ? 'Focus on avoiding careless mistakes and time management.' :
            difficulty === 'medium' ? 'Build deeper understanding of core concepts.' :
          ...
          actionItems: [
            ...
            difficulty === 'easy' ? 'Double-check answers before submitting' : 'Work through step-by-step solutions',
            `Time yourself on ${difficulty} questions`
          ],
          estimatedStudyTime: difficulty === 'easy' ? 20 : difficulty === 'medium' ? 40 : 60,
          relatedQuestions: []
        })
      }
    })
    ...
    if (weaknessAnalysis.timeManagementIssues.tooSlow || weaknessAnalysis.timeManagementIssues.tooFast) {
      recommendations.push({
        type: 'time_management',
        priority: 'medium',
        title: weaknessAnalysis.timeManagementIssues.tooSlow ? 'Improve Time Management' : 'Slow Down and Check Work',
  ...
  static async getPracticeQuestions(
    ...
    const { data: recentCorrect } = await supabase
      ...
      .select('question_id')
      .eq('is_correct', true)
      .gte('answered_at', new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString()) // Last 30 days
  ...
  static async createReviewSession(userId: string, topicFilter?: string): Promise<WrongAnswer[]> {
