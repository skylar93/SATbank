...
export class WeeklyActivityService {
  static async fetchWeeklyActivityData(userId: string): Promise<WeeklyActivityData> {
    try {
      ...
      const weeklyStats: { [key: string]: { studyTime: number, practiceTests: number } } = {
      ...
      attempts?.forEach(attempt => {
        ...
        if (attempt.status === 'completed' && attempt.completed_at) {
          ...
          const duration = this.calculateDurationInHours(attempt.started_at, attempt.completed_at)
  ...
  private static calculateDurationInHours(startTime: string, endTime: string): number {
