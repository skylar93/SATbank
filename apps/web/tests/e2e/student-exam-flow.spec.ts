...
test.describe('Student Exam Flow', () => {
  ...
  test('student can complete full exam flow from login to results', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /available exams/i })).toBeVisible()
    ...
    for (let questionNum = 1; questionNum <= firstModuleQuestions; questionNum++) {
    ...
    if (await reviewButton.isVisible()) {
      ...
      await expect(page.locator('[data-testid="question-review"]').first()).toBeVisible()
    }
  })
