...
export function PracticeProgress({ timeframe = 'month', showDetailedStats = true }: PracticeProgressProps) {
  const { user } = useAuth()
  ...
  const processPracticeData = async (sessions: any[]): Promise<PracticeStats> => {
    // Basic stats
    const totalPracticeSessions = sessions.length
    const totalQuestionsAnswered = sessions.reduce((sum, session) => 
      sum + (session.user_answers?.length || 0), 0
    )
    const totalCorrect = sessions.reduce((sum, session) => 
      sum + (session.user_answers?.filter((a: any) => a.is_correct).length || 0), 0
    )
    const averageAccuracy = totalQuestionsAnswered > 0 ? (totalCorrect / totalQuestionsAnswered) * 100 : 0
    
    const totalTimeSpentSeconds = sessions.reduce((sum, session) => {
      if (session.time_spent?.total) return sum + session.time_spent.total
      return sum + (session.user_answers?.reduce((timeSum: number, answer: any) => 
        timeSum + (answer.time_spent_seconds || 0), 0) || 0)
    }, 0)
    const timeSpentHours = totalTimeSpentSeconds / 3600

    // Topic analysis
    const topicStats = new Map<string, { total: number; correct: number; recent: number; recentCorrect: number }>()
      ...
      session.user_answers?.forEach((answer: any) => {
        answer.questions?.topic_tags?.forEach((topic: string) => {
          if (!topicStats.has(topic)) {
            topicStats.set(topic, { total: 0, correct: 0, recent: 0, recentCorrect: 0 })
          }
    ...
    const strongestTopics = topicArray
      ...
      .slice(0, 5)
      .map(t => ({ topic: t.topic, accuracy: t.accuracy, count: t.totalQuestions }))
  ...
  const getMostFrequentModule = (answers: any[]) => {
    ...
    const mostFrequent = Array.from(moduleCounts.entries())
      .sort((a, b) => b[1] - a[1])[0]
  ...
  const generateAccuracyTrend = (sessions: any[]) => {
    ...
    sessions.forEach(session => {
      ...
      const correct = session.user_answers?.filter((a: any) => a.is_correct).length || 0
  ...
  if (!stats) {
    return (
      <div className="bg-white rounded-lg shadow p-6 text-center">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">No Practice Data</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-2xl font-bold text-blue-600">{stats.totalPracticeSessions}</div>
          <div className="text-sm text-gray-600">Practice Sessions</div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-2xl font-bold text-green-600">{stats.totalQuestionsAnswered}</div>
          <div className="text-sm text-gray-600">Questions Practiced</div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-2xl font-bold text-purple-600">{stats.averageAccuracy.toFixed(1)}%</div>
          <div className="text-sm text-gray-600">Average Accuracy</div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-2xl font-bold text-orange-600">{stats.timeSpentHours.toFixed(1)}h</div>
      ...
      {showDetailedStats && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Strongest Topics */}
            <div className="bg-white rounded-lg shadow p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Strongest Topics</h3>
                  ...
                  {stats.strongestTopics.map((topic, index) => (
                    <div key={topic.topic} className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <span className="ml-3 text-sm font-medium text-gray-900">{topic.topic}</span>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-medium text-green-600">{topic.accuracy.toFixed(1)}%</div>
                        <div className="text-xs text-gray-500">{topic.count} questions</div>
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-gray-500 text-sm">Practice more to see your strongest topics.</p>
              )}
            </div>
                ...
                <div className="space-y-3">
                  {stats.weakestTopics.map((topic) => (
                    <div key={topic.topic} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-sm font-medium text-red-600">{topic.accuracy.toFixed(1)}%</div>
                        <div className="text-xs text-gray-500">{topic.count} questions</div>
                      </div>
                    </div>
                  ))}
          ...
          {/* Recent Sessions */}
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Recent Practice Sessions</h3>
            {stats.recentSessions.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Focus</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {stats.recentSessions.map((session) => (
                      <tr key={session.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-sm text-gray-900">{session.question_count}</td>
                        <td className="px-4 py-2 text-sm">
                          <span className={`font-medium ${
                            session.total_score >= 80 ? 'text-green-600' :
                            session.total_score >= 60 ? 'text-yellow-600' : 'text-red-600'
                          }`}>
                            {session.total_score}%
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-2 text-sm text-gray-900">{session.module_focus || 'Mixed'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <p className="text-gray-500 text-sm">No recent practice sessions found.</p>
            )}
          </div>
          ...
          {stats.topicProgress.length > 0 && (
            <div className="bg-white rounded-lg shadow p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Topic Progress Detail</h3>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Recent</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Trend</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {stats.topicProgress.sort((a, b) => b.accuracy - a.accuracy).map((topic) => (
                      <tr key={topic.topic} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-sm">
                          <span className={`font-medium ${
                            topic.recentAccuracy >= 80 ? 'text-green-600' :
                            topic.recentAccuracy >= 60 ? 'text-yellow-600' : 'text-red-600'
                          }`}>
                            {topic.recentAccuracy > 0 ? `${topic.recentAccuracy.toFixed(1)}%` : 'N/A'}
                          </span>
                        </td>
                        <td className="px-4 py-2 text-sm">
                          {topic.trend === 'improving' && <span className="text-green-600">↗️ Improving</span>}
                          {topic.trend === 'declining' && <span className="text-red-600">↘️ Declining</span>}
                          {topic.trend === 'stable' && <span className="text-gray-600">→ Stable</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
