...
interface IncorrectAnswersSectionProps {
  questions: Question[]
  onRefresh: () => void
}

export function IncorrectAnswersSection({ questions, onRefresh }: IncorrectAnswersSectionProps) {
  const { user } = useAuth()
  const supabase = createClient()
  const [detailedQuestions, setDetailedQuestions] = useState<Question[]>([])
  const [loading, setLoading] = useState(true)
  const [groupBy, setGroupBy] = useState<'module' | 'difficulty' | 'topic' | 'recent'>('recent')
  const [expandedQuestion, setExpandedQuestion] = useState<string | null>(null)

  useEffect(() => {
    if (user && questions.length > 0) {
      fetchDetailedIncorrectAnswers()
    }
  }, [user, questions])

  const fetchDetailedIncorrectAnswers = async () => {
    try {
      setLoading(true)
      
      const incorrectQuestionIds = questions.filter(q => q.is_incorrect).map(q => q.id)
      ...
      // Fetch detailed incorrect answers for these questions
      ...
      setDetailedQuestions(Array.from(questionMap.values()))
    } catch (error) {
      console.error('Error fetching detailed incorrect answers:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const getGroupedQuestions = () => {
    let grouped: { [key: string]: Question[] } = {}
    ...
    switch (groupBy) {
      ...
      case 'difficulty':
        detailedQuestions.forEach(q => {
          const key = q.difficulty_level.charAt(0).toUpperCase() + q.difficulty_level.slice(1)
          if (!grouped[key]) grouped[key] = []
          grouped[key].push(q)
        })
        break
      ...
      case 'recent':
      default:
        grouped['All Incorrect Questions'] = detailedQuestions.sort((a, b) => {
          const aLatest = Math.max(...(a.incorrectAttempts?.map(att => new Date(att.answered_at).getTime()) || [0]))
          const bLatest = Math.max(...(b.incorrectAttempts?.map(att => new Date(att.answered_at).getTime()) || [0]))
  ...
  const generatePracticeFromIncorrect = async () => {
    if (detailedQuestions.length === 0 || !user) return

    try {
      // Create a practice attempt with only incorrect questions
      const { data: attempt, error: attemptError } = await supabase
        ...
        .insert({
          ...
          current_module: detailedQuestions[0]?.module_type,
      ...
      const practiceData = {
        ...
        questions: detailedQuestions.map(q => q.id),
        settings: {
      ...
      window.location.href = `/student/practice/${attempt.id}`
      
    } catch (error) {
      console.error('Error generating practice from incorrect answers:', error)
  ...
  if (detailedQuestions.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow p-6 text-center">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">No incorrect answers yet!</h3>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-medium text-gray-900">
              Incorrectly Answered Questions ({detailedQuestions.length})
            </h3>
          ...
          <div className="flex items-center space-x-3">
            <button
              onClick={generatePracticeFromIncorrect}
              className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md font-medium transition-colors"
            >
              Practice All Incorrect
            </button>
            <button
              onClick={onRefresh}
        ...
        <div key={groupName} className="bg-white rounded-lg shadow">
          ...
          <div className="p-4 space-y-4">
            {groupQuestions.map((question) => (
              <div key={question.id} className="border border-red-200 rounded-lg p-4 bg-red-50">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                        Incorrect {question.incorrectAttempts?.length || 0} time(s)
                      </span>
                    </div>
                    ...
                    {question.incorrectAttempts && question.incorrectAttempts.length > 0 && (
                      <div className="bg-white border border-red-200 rounded p-2 mb-2">
                        <p className="text-xs text-gray-600 mb-1">
                          Latest incorrect answer: <span className="font-medium text-red-600">{question.incorrectAttempts[0].user_answer}</span>
                        </p>
                        <p className="text-xs text-gray-500">
                          {new Date(question.incorrectAttempts[0].answered_at).toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric',
                            hour: '2-digit',
                ...
                {expandedQuestion === question.id && (
                  <div className="mt-4 pt-4 border-t border-red-200">
                    <div className="space-y-4">
                              ...
                              <div
                                ...
                                className={`p-2 rounded ${
                                  key === question.correct_answer
                                    ...
                                    : question.incorrectAttempts?.some(att => att.user_answer === key)
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <div className="p-2 rounded bg-red-100 border border-red-300">
                              <span className="font-medium">{question.incorrectAttempts?.[0]?.user_answer || 'N/A'}</span>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {question.incorrectAttempts && question.incorrectAttempts.length > 1 && (
                        <div>
                          <h5 className="font-medium text-gray-900 mb-2">All Incorrect Attempts:</h5>
                          <div className="space-y-2">
                            {question.incorrectAttempts.map((attempt, index) => (
                              <div key={attempt.id} className="bg-red-50 border border-red-200 rounded p-2">
                                <div className="flex justify-between items-center">
                                  <span className="text-sm">
                                    <span className="font-medium">Answer:</span> {attempt.user_answer}
                                  </span>
                                  <span className="text-xs text-gray-500">
                                    {new Date(attempt.answered_at).toLocaleDateString('en-US', {
                                      year: 'numeric',
                                      month: 'short',
                                      day: 'numeric',
                                      hour: '2-digit',
